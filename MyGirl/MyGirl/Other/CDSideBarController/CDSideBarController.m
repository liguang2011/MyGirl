...
//        [button setImage:image forState:UIControlStateNormal];
        ...
        button.frame = CGRectMake(20, 50 + (80 * index), 170, 50);
        button.tag = index;
...
- (void)performOpenAnimation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.4 animations:^{
            ...
            _menuButton.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -190, 0);
            _backgroundMenuView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -190, 0);
        }];
    });
    for (UIButton *button in _buttonList)
    {
